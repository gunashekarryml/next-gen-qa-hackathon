import type { FailureRecord, Classification } from './classifier.js';

export function generateReasoning(rec: FailureRecord, c: Classification) {
  const parts: string[] = [];
  if (c.explain.matched.length) parts.push(`Matched: ${c.explain.matched.join(', ')}`);
  if (rec.error_type) parts.push(`ErrorType: ${rec.error_type}`);
  if (rec.test_id) parts.push(`Test: ${rec.test_id}`);
...
function recommendShort(category: string) {
  switch (category) {
...
function recommendLong(category: string) {
  switch (category) {
